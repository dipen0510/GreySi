...
- (void)setTitleWithAnnotation:(id<MultiRowAnnotationProtocol>)annotation
{
    if (!_titleLabel)
    {
        ...
        _subtitleLabel.font = [UIFont systemFontOfSize:12.0 weight:UIFontWeightThin];
        ...
        _budgetLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        _budgetLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        _budgetLabel.textColor = [UIColor whiteColor];
        _budgetLabel.backgroundColor = [UIColor clearColor];
        _budgetLabel.font = [UIFont systemFontOfSize:12.0 weight:UIFontWeightThin];
        _budgetLabel.shadowColor = [UIColor darkTextColor];
        _budgetLabel.shadowOffset = CGSizeMake(0, -1);
    ...
    if (annotation)
    {
        ...
        _budgetLabel.text = [annotation budget];
        _budgetLabel.hidden = NO;
        _userImgView.hidden = NO;
        
        __weak UIImageView* weakImageView = _userImgView;
        [_userImgView setImageWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[[annotation userImg] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]
...
- (void)prepareContentFrame
{
    ...
    if (_titleLabel)
    {
        _titleLabel.frame = CGRectMake(90, 8, kMultiRowCalloutCellSize.width - 90 - (2*_cellInsetX), 18);
        _subtitleLabel.frame = CGRectMake(90, 34, kMultiRowCalloutCellSize.width - 90 - (2*_cellInsetX), 18);
        _budgetLabel.frame = CGRectMake(90, 60, kMultiRowCalloutCellSize.width - 90 - (2*_cellInsetX), 18);
        _userImgView.frame = CGRectMake(8, 8, 74, 74);
